         ...
         <tbody>
             {data.map((row, idx) => (
                 <tr key={row.id} className={rowStyles.row}>
                     ...
                     {isOffice && (
                         <td className={`border border-gray-400 p-1 overflow-hidden ${rowStyles.text}`}>
                             <div className="truncate px-1" title={row.affiliation}>{row.affiliation || '-'}</div>
                         </td>
                     )}
                     
                     <td className="border border-gray-400 p-1 overflow-hidden">
                         {!isOffice && row.affiliation && <div className="text-[10px] text-gray-500 truncate">{row.affiliation}</div>}
                         <div className={`font-medium truncate ${rowStyles.text}`} title={row.dept}>{row.dept}</div>
                     </td>
                     
                     <td className={`border border-gray-400 p-1 font-bold truncate ${rowStyles.text}`}>{row.name}</td>
